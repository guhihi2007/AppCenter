...
public class ApkSignatureCompare {
    ...
    public static boolean isSignatureSame(Context context, String apkPath, String installedAppPkgName) {
        ...
        LogUtils.i(TAG, "已安装包验证签名耗时：" + String.valueOf((t2 - t1)) + "签名" + installedAppSignature);
        if (TextUtils.isEmpty(installedAppSignature)) {
            // 手机之前没有安装过此应用，所以返回true，不需弹窗
            Log.i(TAG, "installedAppSignature = " + installedAppSignature + "， 手机之前没有安装过此应用，所以返回true，不需弹窗");
            return true;
        }
        ...
        LogUtils.i(TAG, "下载包验证签名耗时：" + String.valueOf((t4 - t3)) + "签名：" + apkSignature);
        if (TextUtils.isEmpty(apkSignature) || TextUtils.isEmpty(installedAppSignature)) {
            // 由于两个签名不能全部获得，无法对比，只能返回true，不需弹窗
            Log.i(TAG, "由于两个签名不能全部获得，无法对比，只能返回true，不需弹窗");
            return true;
        }

        Log.i(TAG, "isSignatureSame = " + apkSignature.equals(installedAppSignature));
