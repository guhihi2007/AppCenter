package cn.lt.android.install.root;
...
public class PackageUtils {
     ...
     * <li>else see {@link #installNormal(Context, String, AppEntity)}</li>
     * </ul>
     *
     * @param context
     * @param filePath
    ...
    public static final int install(Context context, String filePath) {
        ...
        return installNormal(context, filePath, null) ? INSTALL_SUCCEEDED : INSTALL_FAILED_INVALID_URI;   //先传个空
    }
     ...
     */
    public static boolean installNormal(Context context, String filePath, AppEntity appEntity) {
        Intent i = new Intent(Intent.ACTION_VIEW);
        File file = new File(filePath);
        if (!file.exists() || !file.isFile() || file.length() <= 0) {
            //移除轮询器中的监控任务
            if (appEntity != null) InstalledLooperProxy.getInstance().removeLooperEntity();
            return false;
        }

        LogUtils.d("ccc", "安装包路径==>"+filePath);

        if (Build.VERSION.SDK_INT >= 24) {
            i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            Uri contentUri = FileProvider.getUriForFile(context, "cn.lt.appstore.fileProvider", file);
            i.setDataAndType(contentUri, "application/vnd.android.package-archive");
        } else {
            i.setDataAndType(Uri.parse("file://" + filePath), "application/vnd.android.package-archive");
        }
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); //或FLAG_ACTIVITY_PREVIOUS_IS_TOP
        int request_code = LTApplication.instance.requstCode_uuid++;
        if (appEntity != null)
            LTApplication.instance.normalInstallTaskLooper.put(request_code, appEntity);
...
//          com.tencent.android.qqdownloader（应用宝）    com.vmall.client(华为商城)  com.vivo.browser     com.huawei.hidisk    com.huawei.appmarket
    ...
    public static int installSilent(Context context, String filePath, String pmParams) {
        ...
        LogUtils.e(TAG, new StringBuilder().append("installSilent successMsg:").append(commandResult.successMsg).append(", ErrorMsg:").append(commandResult.errorMsg).toString());
